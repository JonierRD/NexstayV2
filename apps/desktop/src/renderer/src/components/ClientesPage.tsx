...
//filtro 
  ...
  return <div className="flex h-full flex-col gap-3 p-4">
    ...
    {history && <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40"><div className="relative max-h-[80vh] w-full max-w-xl overflow-auto rounded-2xl bg-white p-5"><button onClick={() => setHistory(null)} className="absolute right-3 top-3"><X size={18} /></button><h2 className="text-base font-semibold">Historial de {history.firstName} {history.lastName}</h2><div className="mt-3 rounded-xl border border-[#eadfd6] bg-[#fcf8f4] p-4 text-xs"><div className="grid grid-cols-2 gap-x-4 gap-y-2"><p><span className="text-[#7d6d61]">Nombre: </span><span className="font-medium">{history.firstName} {history.lastName}</span></p><p><span className="text-[#7d6d61]">Cédula: </span><span className="font-medium">{history.cc}</span></p><p><span className="text-[#7d6d61]">Teléfono: </span><span className="font-medium">{history.phone || 'Sin teléfono'}</span></p><p><span className="text-[#7d6d61]">Profesión: </span><span className="font-medium">{history.profession || 'Sin profesión'}</span></p><p><span className="text-[#7d6d61]">Ciudad de origen: </span><span className="font-medium">{history.cityOrigin || '-'}</span></p><p><span className="text-[#7d6d61]">Ciudad de destino: </span><span className="font-medium">{history.cityDestination || '-'}</span></p>{history.notes ? <p className="col-span-2"><span className="text-[#7d6d61]">Notas: </span><span className="font-medium">{history.notes}</span></p> : null}</div></div><div className="mt-4 space-y-2">{history.stays?.length ? history.stays.map((stay) => <div key={stay.id} className="rounded-lg border border-[#eadfd6] p-3 text-xs"><div className="flex justify-between font-medium"><span>Habitación {stay.roomNumber}</span><span className={stay.status === 'ACTIVA' ? 'text-[#2f8f4e]' : 'text-[#7d6d61]'}>{stay.status === 'ACTIVA' ? 'ACTIVA' : stay.checkOut ? 'FINALIZADA' : stay.status}</span></div><p className="mt-1 font-medium text-[#2b1b14]">{formatDateRange(stay.checkIn, stay.checkOut)}</p><p className="mt-1 text-[#7d6d61]">{stay.nights} noche(s) · {stay.acTypeUsed === 'AIRE' ? 'Aire' : 'Ventilador'} · ${Number(stay.pricePerNight).toLocaleString('es-CO')}/noche</p><p className="text-[#7d6d61]">Total: <span className="font-medium text-[#2b1b14]">${Number(stay.total).toLocaleString('es-CO')}</span></p></div>) : <p className="text-xs text-[#8d7b70]">Este cliente aún no tiene hospedajes.</p>}</div></div></div>}
