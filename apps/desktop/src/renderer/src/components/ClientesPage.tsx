...
export function ClientesPage({ user: _user }: Props): ReactElement {
  ...
  async function remove(password: string): Promise<void> {
    if (!deleting) return;
    try { await deleteClienteRequest(deleting.id, password); setDeleting(null); await load(); }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'No se pudo eliminar el cliente.'); setDeleting(null); }
  }
  ...
  return <div className="flex h-full flex-col gap-3 p-4">
    ...
    {deleting && <AdminPasswordModal onClose={() => setDeleting(null)} onSuccess={remove} />}
  </div>;
}